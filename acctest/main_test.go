...
func init() {
	...
	flag.StringVar(&cert, "stub-cert", "certificate.pem", "path to the x509 certificate")
	flag.StringVar(&key, "stub-key", "key.pem", "path to the stub server private certificate")
...
func TestShouldFailOnTlsVerificationWithSelfSignedCert(t *testing.T) {
	// given
	srv, _, err := StartServer(port, cert, key)
	if err != nil {
		log.Fatalf("can't start stub server: %v", err)
	}
	defer srv.GracefulStop()
...
func TestShouldBeAbleToSetCustomCA(t *testing.T) {
	// given
	srv, _, err := StartServer(port, cert, key)
	if err != nil {
		log.Fatalf("can't start stub server: %v", err)
	...
	// when
	stdout, stderr, exitcode := runBin(t, "--tls-cert", cert, stubSrvAddr)
